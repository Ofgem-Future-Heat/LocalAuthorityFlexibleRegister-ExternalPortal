...
namespace Ofgem_Web_LAF_ExternalPortal.Pages.Declarations.Change
{
    ...
    public class ReferralLaRemitModel(
        ILogger<ReferralLaRemitModel> logger,
        Services.IDeclarationManagementService declarationManagementService) : PageModel
    {
        ...
        [BindProperty] public string? SelectedLaRemit { get; set; } = string.Empty;
        ...
        {
            logger.LogInformation("ReferralLaRemitModel - OnGetWithId");
            ...
            Urn = Declaration.Urn;
            SelectedLaRemit = Declaration.ReferralMadeOutsideOfLAsRemit;
        }

        public async Task<IActionResult> OnPost()
        {
            logger.LogInformation("ReferralLaRemitModel - OnPost");

            if (string.IsNullOrEmpty(SelectedLaRemit))
            {
            ...
            if (SelectedLaRemit != AnswerYes && SelectedLaRemit != AnswerNo)
            {
            ...
            await RefreshData(DeclarationId, SelectedLaRemit);
            ...
            Declaration.ReferralMadeOutsideOfLAsRemit = SelectedLaRemit;
        ...
        private async Task RefreshData(Guid declarationId, string? preSelectedLaRemit)
        {
            try
            {
                ...
                {
                    SelectedLaRemit = preSelectedLaRemit;
                }
            }
            catch (Exception ex)
            {
